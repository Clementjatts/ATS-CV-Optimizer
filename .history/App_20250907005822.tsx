...
export default function App() {
  ...
  const handleSaveAsPdf = () => {
    ...
    if (element && optimizedCvData) {
        ...
        // Remove any responsive classes and apply PDF-specific styling
        const allElements = pdfContent.querySelectorAll('*');
        allElements.forEach((el: Element) => {
          const htmlEl = el as HTMLElement;
          // Remove responsive padding/margins
          htmlEl.style.padding = htmlEl.style.padding?.replace(/\d+px|\d+rem|\d+em/g, '0') || '';
          htmlEl.style.margin = htmlEl.style.margin?.replace(/md:p-\d+|lg:p-\d+|p-\d+/g, '') || '';
        });
        ...
        // Generate PDF with better timing
        setTimeout(() => {
