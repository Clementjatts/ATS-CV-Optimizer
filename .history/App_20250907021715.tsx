...
export default function App() {
  ...
  const generatePdfPreview = async () => {
    ...
    try {
      ...
      const { generatePdfFilename } = await import('./services/pdf-filename-generator');
      
      // Create PDF generation controller
      const controller = new PDFGenerationController();
      
      // Generate filename
      const filename = generatePdfFilename(optimizedCvData);

      // Create PDF options (use medium quality for preview to be faster)
      const pdfOptions = {
        filename,
        ...
        metadata: {
          title: `${optimizedCvData.fullName} - Professional CV (Preview)`,
          author: optimizedCvData.fullName,
          subject: 'Professional CV Preview',
          keywords: ['CV', 'Resume', 'Professional', optimizedCvData.fullName],
      ...
      const result = await controller.generatePDF(element, pdfOptions);
  ...
  const handleSaveAsPdf = async () => {
    ...
    try {
      ...
      const pdfOptions = {
        ...
        metadata: {
          title: `${optimizedCvData.fullName} - Professional CV`,
          author: optimizedCvData.fullName,
      ...
      setPdfGenerationStatus(prev => ({
        ...prev,
        progress: 'Generating high-quality PDF...'
      }));
      ...
      if (result.success && result.blob) {
        ...
        setPdfGenerationStatus({
          isGenerating: false,
          progress: `PDF generated successfully! Quality: ${pdfQuality.toUpperCase()}, Size: ${(result.blob.size / 1024 / 1024).toFixed(2)} MB`,
          error: null
        });
             ...
             <div className="flex-grow bg-slate-200 rounded-md border border-slate-300 min-h-[40rem] flex flex-col overflow-hidden">
              ...
              {!isLoading && !error && optimizedCvData && (
                <>
                  ...
                  <div className="mt-4 p-4 border-t border-slate-300 bg-white">
                    {/* PDF Generation Status */}
                    {(pdfGenerationStatus.isGenerating || pdfGenerationStatus.progress || pdfGenerationStatus.error) && (
                      <div className="mb-4">
                        {pdfGenerationStatus.isGenerating && (
                          <div className="flex items-center gap-2 text-indigo-600 bg-indigo-50 p-3 rounded-md">
                            <LoadingSpinner className="h-4 w-4" />
                            <span className="text-sm font-medium">{pdfGenerationStatus.progress}</span>
                          </div>
                        )}
                        {pdfGenerationStatus.progress && !pdfGenerationStatus.isGenerating && !pdfGenerationStatus.error && (
                          ...
                          <div className="text-red-600 bg-red-50 border border-red-200 p-4 rounded-md">
                            <div className="flex items-start gap-2">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-xs mt-1">{pdfGenerationStatus.error}</p>
                                  ...
                                  <div className="space-y-1 text-xs">
                                    <button
                                      onClick={() => {
                                        ...
                                        setPdfGenerationStatus(prev => ({ ...prev, error: null }));
                                      }}
                                      className="block w-full text-left px-2 py-1 bg-red-100 hover:bg-red-200 rounded text-red-800 transition-colors"
                                    >
                                      • Try with Medium quality (reduces memory usage)
                                    </button>
                                    <button
                                      onClick={() => {
                                        setPdfQuality('low');
                                      ...
                                      className="block w-full text-left px-2 py-1 bg-red-100 hover:bg-red-200 rounded text-red-800 transition-colors"
                                    >
                                      • Try with Low quality (fastest generation)
                                    </button>
                                    <button
                                      onClick={() => {
                    ...
                    <div className="mb-4">
                      ...
                      >
                        <svg className={`h-4 w-4 transition-transform ${showPdfOptions ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500 mt-2">
                                Higher quality produces larger files but better print results
                              </p>
                            </div>
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
                              <div className="flex items-start gap-2">
                                ...
                                <div className="text-sm text-blue-800">
                                  <p className="font-medium mb-1">Quality Guide:</p>
                                  <ul className="space-y-1 text-xs">
                                    <li><strong>High:</strong> Best for printing and professional submission (larger file)</li>
                                    <li><strong>Medium:</strong> Good balance for most applications (recommended)</li>
                                    <li><strong>Low:</strong> Smaller file size for email or quick sharing</li>
                                  </ul>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="bg-green-50 border border-green-200 rounded-md p-3">
                              <div className="flex items-start gap-2">
                                <SparkleIcon className="h-4 w-4 text-green-600 mt-0.5 flex-shrink-0" />
                                <div className="text-sm text-green-800">
                                  <p className="font-medium mb-1">Advanced PDF Features:</p>
                                  <ul className="space-y-1 text-xs">
                                    <li>• <strong>ATS Optimization:</strong> Invisible text layer for perfect parsing</li>
                                    <li>• <strong>Professional Fonts:</strong> Embedded fonts for consistent display</li>
                                    <li>• <strong>Vector Graphics:</strong> Crisp borders and shapes at any zoom</li>
                                    <li>• <strong>Smart Compression:</strong> Optimal file size without quality loss</li>
                                    <li>• <strong>Cross-Platform:</strong> Works on all devices and PDF viewers</li>
                                  </ul>
                                </div>
                              </div>
                            </div>
                    ...
                    {showPreview && pdfPreview.url && (
                      <div className="mb-4 p-4 bg-slate-50 border border-slate-200 rounded-md">
                        <div className="flex items-center justify-between mb-3">
                          <h3 className="text-sm font-medium text-slate-800">PDF Preview</h3>
                          <button
                            onClick={() => {
                              setShowPreview(false);
                              if (pdfPreview.url) {
                        ...
                        {pdfPreview.quality && (
                          <div className="mb-3 p-3 bg-white border border-slate-200 rounded-md">
                            <h4 className="text-xs font-medium text-slate-700 mb-2">Quality Metrics</h4>
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              <div>
                                <span className="text-slate-500">File Size:</span>
                                <span className="ml-1 font-medium">
                                  {pdfPreview.blob ? (pdfPreview.blob.size / 1024 / 1024).toFixed(2) : '0'} MB
                                </span>
                              </div>
                              <div>
                                <span className="text-slate-500">Generation Time:</span>
                                <span className="ml-1 font-medium">
                                  {pdfPreview.quality.duration ? (pdfPreview.quality.duration / 1000).toFixed(1) : '0'}s
                                </span>
                              </div>
                              <div>
                                <span className="text-slate-500">Strategy:</span>
                                <span className="ml-1 font-medium capitalize">
                                  {pdfPreview.quality.strategy || 'Modern'}
                                </span>
                              </div>
                              <div>
                                <span className="text-slate-500">Pages:</span>
                                <span className="ml-1 font-medium">
                                  {pdfPreview.quality.pageCount || 1}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="mt-3 space-y-2">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-green-700">ATS Compatible Format</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <CheckCircleIcon className="h-4 w-4 text-green-600" />
                                <span className="text-xs text-green-700">Professional Layout Preserved</span>
                              </div>
                              {pdfPreview.blob && pdfPreview.blob.size < 2 * 1024 * 1024 && (
                                <div className="flex items-center gap-2">
                                  <CheckCircleIcon className="h-4 w-4 text-green-600" />
                                  <span className="text-xs text-green-700">Optimal File Size (&lt; 2MB)</span>
                                </div>
                              )}
                              {pdfPreview.blob && pdfPreview.blob.size >= 2 * 1024 * 1024 && (
                                <div className="flex items-center gap-2">
                                  <InfoIcon className="h-4 w-4 text-yellow-600" />
                                  <span className="text-xs text-yellow-700">Large file size - consider lower quality</span>
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="mt-3 flex gap-2">
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1 text-xs bg-indigo-600 text-white rounded hover:bg-indigo-700 transition-colors"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1 text-xs bg-slate-200 text-slate-700 rounded hover:bg-slate-300 transition-colors disabled:opacity-50"
                          >
                            Regenerate
                          </button>
                        </div>
                    ...
                    <div className="flex gap-4">
                      <button 
                        onClick={handleCopy} 
                        className="flex items-center gap-2 bg-slate-200 text-slate-800 font-medium py-2 px-4 rounded-md hover:bg-slate-300 transition-colors"
                      >
                      ...
                      <button 
                        onClick={generatePdfPreview}
                        disabled={pdfGenerationStatus.isGenerating}
                        className="flex items-center gap-2 bg-slate-600 text-white font-medium py-2 px-4 rounded-md hover:bg-slate-700 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors"
                      >
                        {pdfGenerationStatus.isGenerating ? (
                          <>
                            <LoadingSpinner className="h-4 w-4" />
                      ...
                      <button 
                        ...
                        disabled={pdfGenerationStatus.isGenerating}
                        className="flex items-center gap-2 bg-indigo-600 text-white font-medium py-2 px-4 rounded-md hover:bg-indigo-700 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors"
                      >
                        {pdfGenerationStatus.isGenerating ? (
                          <>
                            <LoadingSpinner className="h-4 w-4" />
