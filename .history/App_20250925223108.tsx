...
export default function App() {
  ...
  React.useEffect(() => {
    ...
    if (document.readyState === 'complete') {
      ...
      return () => window.removeEventListener('load', checkLibrary);
    }
  }, []);
  ...
  const generatePdfPreview = async () => {
    ...
    try {
      // Check if html2pdf.js is available with retry logic
      let html2pdf = (window as any).html2pdf;

      if (!html2pdf) {
        // Wait a bit more and try again in case the library is still loading
        setPdfGenerationStatus({
          isGenerating: true,
      ...
      // High-quality PDF generation settings
      const opt = {
        margin: 0.5,
        filename: `${optimizedCvData.fullName.replace(/\s+/g, '_')}_CV_Optimised.pdf`,
        image: { 
          type: 'png', // PNG for better text quality
          quality: 1.0  // Maximum quality
        },
        html2canvas: {
          scale: 3, // Higher scale for crisp text (3x for high DPI)
          useCORS: true,
          allowTaint: true,
          ...
          imageTimeout: 15000, // Longer timeout for better quality
            ...
            {optimizedCvData && (
              <div className="mt-6">
                <button
                  ...
                  disabled={pdfGenerationStatus.isGenerating || !isPdfLibraryReady}
