...
export default function App() {
  ...
  const generatePdfPreview = async () => {
    ...
    try {
      ...
      if (!html2pdf) {
        // Wait a moment for the library to load and try again
        await new Promise(resolve => setTimeout(resolve, 1000));
        const html2pdfRetry = (window as any).html2pdf;
        if (!html2pdfRetry) {
          throw new Error('PDF library not loaded. Please refresh the page and try again.');
        }
        // Use the retry reference
        const pdfBlob = await html2pdfRetry().set(opt).from(element).outputPdf('blob');
