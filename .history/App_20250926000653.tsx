...
Please provide a modified version that incorporates the user's request while keeping the same structure and format. Only change what the user specifically requested.`;
  ...
  const generatePdfPreview = async () => {
    ...
    try {
      ...
      // Ultra high-quality PDF generation settings
      const opt = {
        margin: 0.4,
        filename: `${optimizedCvData.fullName.replace(/\s+/g, '_')}_CV_Optimised.pdf`,
        image: { 
          type: 'png', // PNG for better text quality than JPEG
          quality: 1.0  // Maximum quality
        },
        html2canvas: {
          scale: 4, // Ultra high scale for maximum sharpness (4x for ultra high DPI)
          useCORS: true,
          allowTaint: true,
          ...
          imageTimeout: 30000, // Extended timeout for high quality rendering
          onclone: (clonedDoc: Document) => {
            // Enhance fonts and rendering in cloned document for maximum quality
            const clonedElement = clonedDoc.getElementById('cv-preview');
            if (clonedElement) {
              clonedElement.style.fontFamily = 'Calibri, "Helvetica Neue", Arial, sans-serif';
        ...
        jsPDF: {
          ...
          floatPrecision: 'smart' // Smart precision for better quality
        },
        pagebreak: {
          mode: ['avoid-all', 'css', 'legacy'], // Better page break handling
          avoid: ['.cv-list li', 'h1', 'h2', 'h3', 'section'] // Avoid breaking these elements
        }
      };
