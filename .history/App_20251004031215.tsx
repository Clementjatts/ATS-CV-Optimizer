...
export default function App() {
  ...
  const handleFileChange = async (file: File) => {
    ...
    try {
      // Auto-save the uploaded file to Firebase Storage
      await fileStorageService.uploadFile(file, `Uploaded CV - ${file.name}`);
    ...
    try {
      ...
      if (useDatabaseCV && selectedCVFromDB) {
        ...
        if ('content' in selectedCVFromDB) {
          ...
          // It's an uploaded file - we need to parse it
          const uploadedFile = selectedCVFromDB as UploadedFile;
          // For now, we'll need to re-parse the file or store the text
          // This is a limitation - we'd need to store the parsed text too
          throw new Error('Cannot optimize from uploaded file directly. Please upload the file first.');
        }
