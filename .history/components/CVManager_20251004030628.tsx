...
import { cvService, SavedCV, CVSearchFilters } from '../services/cvService';
...
const CVManager: React.FC<CVManagerProps> = ({ onSelectCV, onClose, isOpen }) => {
  ...
  const filterCVs = () => {
    ...
    // Apply filters
    if (filters.jobTitle) {
      filtered = filtered.filter(cv => cv.jobTitle === filters.jobTitle);
    }
    if (filters.industry) {
      filtered = filtered.filter(cv => cv.industry === filters.industry);
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCVs.map((cv) => (
                <div key={cv.id} className="bg-gradient-to-br from-white to-purple-50 border border-purple-200 rounded-xl p-4 hover:shadow-lg transition-all duration-300 hover:scale-105">
                  ...
                  >
                    {cv.isOriginalUpload ? 'Use This Uploaded CV' : 'Use This CV'}
                  </button>
                </div>
              ))}
            </div>
