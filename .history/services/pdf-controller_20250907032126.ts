...
import { PDFPerformanceOptimizer, PDFGenerationCache } from './pdf-performance-optimizer';
...
export class PDFGenerationController {
  ...
  private performanceOptimizer: PDFPerformanceOptimizer;
  private performanceMonitor: PDFPerformanceMonitor;
  private generationCache: PDFGenerationCache;

  constructor() {
    this.detectBrowserCapabilities();
    this.errorRecoveryService = new PDFErrorRecoveryService();
    this.performanceOptimizer = new PDFPerformanceOptimizer();
    this.performanceMonitor = new PDFPerformanceMonitor();
    this.generationCache = this.performanceOptimizer.setupGenerationCache();
  }
  ...
  async generatePDF(element: HTMLElement, options: PDFOptions): Promise<PDFResult> {
    ...
    try {
      ...
      // Lazy load dependencies for the selected strategy
      await this.performanceOptimizer.lazyLoadDependencies(strategy);
      
      // Create full configuration
      let config = this.createConfiguration(options, strategy);
      
      // Optimize configuration for performance
      const optimization = this.performanceOptimizer.optimizeConfiguration(
        element, 
        config,
        { maxTime: 10000, maxMemory: 100 * 1024 * 1024 }
      );
      
      if (optimization.expectedImprovement > 0.2) {
        config = optimization.optimizedConfig;
        console.log('Applied performance optimizations:', optimization.appliedOptimizations);
      }
      ...
      await this.performanceOptimizer.profileGeneration(
        element,
        config,
      ...
      this.performanceMonitor.trackGeneration(
        strategy,
        config,
        result,
        element,
  ...
  getPerformanceStats(): {
    ...
    return {
      ...
      dashboard: this.performanceMonitor.createPerformanceDashboard(),
      optimizerStats: this.performanceOptimizer.getPerformanceStats()
    };
  }
  ...
  clearPerformanceData(): void {
    ...
    this.generationCache.clearAll();
  }

  /**
   * Configure performance monitoring
   */
  configurePerformanceMonitoring(options: {
    enabled?: boolean;
    alertThresholds?: any;
  }): void {
    if (options.enabled !== undefined) {
      this.performanceMonitor.setMonitoringEnabled(options.enabled);
    }
    
    if (options.alertThresholds) {
      this.performanceMonitor.setAlertThresholds(options.alertThresholds);
    }
  }
   ...
   */
  getCacheStats(): any {
    return this.generationCache.getStats();
  }

  /**
   * Suggest garbage collection if needed
   */
  shouldTriggerGarbageCollection(): boolean {
    return this.performanceOptimizer.suggestGarbageCollection();
  }
  ...
  private detectMaxCanvasSize(): number {
    ...
    if (!ctx) return MEMORY_LIMITS.minCanvasSize as number;
    
    // Test progressively larger canvas sizes
    let maxSize: number = MEMORY_LIMITS.minCanvasSize;
    ...
    return Math.min(maxSize, 32767) as number;
  }
  ...
  private async generateWithModernStrategy(element: HTMLElement, config: PDFConfiguration): Promise<PDFResult> {
    const { ModernPDFGenerator } = await import('./pdf-modern-generator');
    const generator = new ModernPDFGenerator(config);
    
    // Convert config to PDFOptions format
    const options: PDFOptions = {
      filename: 'cv.pdf', // This will be overridden by the caller
      quality: config.resolution >= 300 ? 'high' : config.resolution >= 200 ? 'medium' : 'low',
      format: config.format,
      margins: config.margins,
      metadata: config.metadata
    };
    
    return await generator.generatePDF(element, options);
  }
  ...
  private async generateWithLegacyStrategy(element: HTMLElement, config: PDFConfiguration): Promise<PDFResult> {
    ...
    try {
      ...
      const opt = {
        ...
        filename: 'cv.pdf',
        image: { type: 'jpeg', quality: config.imageQuality },
        html2canvas: { 
          scale: config.resolution / 96,
          useCORS: true,
          scrollX: 0,
          scrollY: 0,
          windowWidth: 1200,
      ...
      const pdfBlob = await new Promise<Blob>((resolve, reject) => {
        html2pdf().set(opt).from(pdfElement).outputPdf('blob').then((blob: Blob) => {
          resolve(blob);
        }).catch(reject);
      });
      ...
      return {
        ...
        metadata: {
          ...
          pageCount: 1,
          warnings: ['Using legacy PDF generation strategy']
        }
      };
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Legacy PDF generation failed',
        metadata: {
          strategy: 'legacy',
          duration,
          fileSize: 0,
          pageCount: 0,
          warnings: ['Legacy strategy failed']
        }
      };
    }
  }
