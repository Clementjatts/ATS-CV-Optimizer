...
export class ModernPDFGenerator {
  ...
  async generatePDF(element: HTMLElement, options: PDFOptions): Promise<PDFResult> {
    ...
    try {
      ...
      const fontLoadResult = await this.loadFontsWithCache(detectedFonts.map(f => f.family));
      ...
      this.addCanvasImageToPDF(pdf, canvas, options);
  ...
  private async calculateQualityMetrics(
    ...
    const fileSizeCompliant = blob.size <= (5 * 1024 * 1024); // 5MB limit
  ...
  private async loadFontsWithCache(detectedFonts: string[]): Promise<any> {
    ...
    if (fontsToLoad.length > 0) {
      // Convert strings to FontInfo objects for the font manager
      const fontInfos = fontsToLoad.map(family => ({ family, weight: 'normal', style: 'normal' }));
      newFonts = await this.fontManager.loadFontsForPDF(fontInfos as any);
