...
const apiKey = import.meta.env.VITE_GEMINI_API_KEY || (process.env as { API_KEY?: string }).API_KEY;
if (!apiKey) {
...
const cvSchema = {
  ...
  properties: {
    fullName: {
      type: Type.STRING,
      description:
        "The candidate's full name EXACTLY as it appears in the original CV - preserve exact spelling and formatting.",
    },
    contactInfo: {
      type: Type.OBJECT,
      properties: {
    ...
    professionalSummary: {
      type: Type.STRING,
      description:
        'A compelling 3-4 sentence professional summary tailored to the job description, highlighting key achievements and value proposition.',
    },
    workExperience: {
      type: Type.ARRAY,
      items: {
        ...
        properties: {
          jobTitle: {
            type: Type.STRING,
            description:
              "Job title optimized for the target role. Extract only the PRIMARY job title. If multiple titles are provided (e.g., 'Security Operative | Transferable Skills for Residential Support'), return only the main role (e.g., 'Security Operative'). Ignore secondary roles, transferable skills, or additional qualifications.",
          },
          ...
          responsibilities: {
            type: Type.ARRAY,
            description:
              '4-6 bullet points of key achievements and responsibilities, optimized with keywords from the job description. Use action verbs and quantify results where possible.',
    ...
    skills: {
      type: Type.ARRAY,
      description:
        'A comprehensive list of technical and soft skills, prioritized based on the job description requirements. DO NOT include certifications, licenses, or qualifications - these belong in the certifications section only.',
      items: { type: Type.STRING },
    },
    optimizationDetails: {
      type: Type.OBJECT,
      ...
      properties: {
        keywordsIntegrated: {
          type: Type.ARRAY,
          description:
            'List of specific keywords from the job description that were integrated into the CV',
          items: { type: Type.STRING },
        },
        skillsAligned: {
          type: Type.ARRAY,
        ...
        summaryTailoring: {
          type: Type.STRING,
          description:
            'Specific explanation of how the professional summary was tailored to the position',
        },
      },
      required: [
        'keywordsIntegrated',
...
export async function extractTextFromImagesWithGemini(base64Images: string[]): Promise<string> {
  const prompt =
    'You are an Optical Character Recognition (OCR) expert. Extract all text content from these document pages in the order they are provided. Combine the text from all pages into a single block of text. Preserve the original structure, paragraphs, and line breaks as best as possible.';
...
export const enhanceCVWithGemini = async (
  cv: string,
  jobDescription: string
): Promise<{ title: string; cvData: CvData | null }> => {
  const prompt = `  
